...
| OQ-011 | Are primăria pilot compartimente care trebuie separate (un `staff` de Urbanism să NU vadă sesizările de Asistență Socială)? | Domain | Reprezentantul primăriei | Open | FUP-14. În Phase 1, **orice `staff` vede toate sesizările tenantului** — [ADR-0003](../decisions/ADR-0003-authentication-and-role-model.md). Dacă răspunsul e „da", este schimbare de model, nu setare. |
| OQ-012 | Cât durează access token-ul și este MFA obligatoriu pentru `tenant_admin`? | Architecture | Product owner + Solution architect | Open | FUP-15. Durata token-ului **determină direct** fereastra de revocare a rolului — vezi [R-005](./risk-register.md) și T42 din [ADR-0003](../decisions/ADR-0003-authentication-and-role-model.md). Pentru MFA există **argument**, nu decizie. |
